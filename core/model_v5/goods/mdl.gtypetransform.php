 ...
 *  类型转换
 */
class mdl_gtypetransform extends shopObject{
    ...
    */
    function typeTransform($sourceTypeId , $destTypeId, &$goods, $sourceTypeData = null, $destTypeData = null, $transInShopex = true ){
        ...
        //将数据进行转换
        $this->specTransform($goods,$transInShopex);
    ...
    **/
    function &specTransform(&$goods,$transInShopex){
        if( $goods['spec_desc'] && $this->specTransformState == 'contain' ){
            ...
            foreach( $goods['spec_desc'] as $t_specId => $pSpec ){
                foreach( $pSpec as $pSpecId => $pSpecValue ){
                    ...
                    $goodsSpecDesc[$this->specTransformHash[$t_specId]['spec_id']][($transInShopex?$pSpecId:$pSpecValue['spec_value_id'])] = $pSpecValue;
                }
            }
            ...
            foreach( $goods['products'] as $pid => $pro ){
                ...
                if( !$transInShopex ){
                    $aProProps['spec_private_value_id'] = $aProProps['spec_value_id'];
                }
        ...
        if( !$transInShopex ){
            $extSpecDesc = array();
            foreach( $this->destType['spec'] as $aspec ){
                foreach( $aspec['options'] as $aspeck => $aspecv ){
                    $extSpecDesc[$aspec['spec_id']][$aspeck] = $aspecv;
                    $extSpecDesc[$aspec['spec_id']][$aspeck]['spec_type'] = $aspec['spec_type'];
                }
            }
            $goods['spec_desc'] = $extSpecDesc;
            $goods['spec_desc_str'] = urlencode( serialize( $extSpecDesc ) );
        }
        return $this;
    }
