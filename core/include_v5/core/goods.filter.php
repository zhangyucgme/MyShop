...
function goods_filter(&$filter, &$object){
    ...
    if($filter['supplier_id']){ //根据供应商进行搜索
        if(!is_array($filter['supplier_id'])){
                $where[] = ' supplier_id = '.$filter['supplier_id'].' ';

                if(isset($filter['costsync']) && $filter['costsync']) { // �ɱ���ͬ����Ķ����б� wubin 2009-09-14 16:29:24
                     // 获取成本价同步最大版本号
    ...
    if(isset($filter['tag']) && is_array($filter['tag'])){
     ...
     if(isset($filter['props']) && is_array($filter['props'])){/*扩展属性*/
